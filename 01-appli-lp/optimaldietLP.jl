...
data = CSV.read(string(path, /, filename); missingstring = "NA")

nbCommodities = count(typeof(data[:Commodity][i]) == String for i in 1:nrow(data))-1

names = data[:Commodity][1:nbCommodities]

mat = Array(data[1:nrow(data), 3:13]) # Selecting the data to be able to do matrix operations
mat = collect(Missings.replace(mat, 0.0)) # Replacing NAs

# Calling Gurobi : optimization model
...
@variable(m, x[1:nbCommodities]) # optimization variable
@objective(m, Min, c'*x) # objective function
@constraint(m, N*x .>= d) # diet constraint
@constraint(m, x .>= 0.0) # positivity constraint
status = solve(m)

q = getvalue(x) # optimizer
obj = getobjectivevalue(m) # value of the objective at minimum

q_yearly = q*365 # tanslating price to yearly price

# setting up our optimal solution

toKeep = find(!iszero, q_yearly)
foods = q_yearly[toKeep]
foodnames = names[toKeep] # selecting the optimizing foods names
diet = DataFrame(food = foodnames, yearly_cost = foods)
totc = q_yearly'*c # computing total cost

# setting up Stigler's optimal solution
